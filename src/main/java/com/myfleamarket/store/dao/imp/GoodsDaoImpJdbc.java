...
import com.myfleamarket.db.core.JdbcTemplate;
import com.myfleamarket.db.core.PreparedStatementCreator;
import com.myfleamarket.db.core.RowCallbackHandler;
...
public class GoodsDaoImpJdbc implements GoodsDao {
    ...
    public Goods findByPk(long pk) {
        ...
        String sql = "select id,name,price,description,brand,cpu_brand,cpu_type," +
                "memory_capacity,hd_capacity,card_model,displaysize,image from Goods where id=?";
    ...
    public List<Goods> findAll() {
        List<Goods> list = new ArrayList<>();
        String sql = "select id,name,price,description,brand,cpu_brand,cpu_type," +
                "memory_capacity,hd_capacity,card_model,displaysize,image from Goods";
        jdbcTemplate.query(conn -> {
            PreparedStatement ps = conn.prepareStatement(sql);
            return ps;
        }, rs -> populate(list, rs));
    ...
    public void create(Goods goods) {
        String sql = "insert into Goods (id,name,price,description,brand,cpu_brand," +
    ...
    public void modify(Goods goods) {
        String sql = "update Goods set name=?,price=?,description=?,brand=?," +
                "cpu_brand=?,cpu_type=?,memory_capacity=?,hd_capacity=?,card_model=?," +
                "displaysize=?,image=? where id=?";
